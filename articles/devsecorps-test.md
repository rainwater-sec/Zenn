...
## 目的
Dockerの学習をしたかったため、自身が学びたいセキュリティ分野に関する実践的な演習を行い理解を深める。
...
この状態でGithubへコードをpushしようとすると、yamlファイルで設定した通りにCI/CDパイプラインがデプロイをブロックすることを確認できた。

具体的には、Github Actionsに以下のようなエラーメッセージが表示されていた。

```text
Running Trivy with options: trivy image secure-nginx:0862a6...
2026-04-03T06:22:08Z	INFO	Detected OS	family="debian" version="9.8"

(前略)

Total: 77 (HIGH: 50, CRITICAL: 27)
...
これにより、脆弱なコンテナが外部に公開されるリスクを機械的に排除できることが実証された。

この状態でGithubにpushすると、問題なくパイプラインが動作していることが確認できた。
...
まず、検証内容を簡潔な表にすると以下のようになる。

 |比較項目|修正前|修正後|改善効果と実務的な運用対応|
 |---|---|---|---|
|ベースイメージ|nginx:1.14 (Debian 9.8)|nginx:alpine<br>(Alpine 3.23.3)|Alpine採用により攻撃面を大幅削減|
|脆弱性総数|31件|4件|全体のリスクを劇的に削減|
|CRITICAL（致命的）|31件|0件|パイプラインを停止させる致命的なリスクを完全排除|
|HIGH（高）|0件|3件|公式リポジトリ未修正のため `.trivyignore` にてリスク受容|
|MEDIUM（中）|0件|1件|同上で、運用による受容プロセスを適用|
|CI/CDの挙動|エラー終了（デプロイ遮断）|正常に終了（デプロイ成功）| GitHub Actionsにて自動ブロックと通過を実証|
...
したがって、脆弱性の内容をきちんと審査し、許容可能なリスクに対しては `.trivyignore` 等を用いて意図的に無視するリスク受容の運用設計が不可欠であると結論付けられる。
