...
前回記事：https://zenn.dev/rw_sec/articles/nullbyte_sql_injection
...
```
結果として`192.168.56.110`の80番ポートと7744番ポートが開いていることが分かり、こちらがDC-2のIPアドレスであると判明しました。7744番ポートはSSHサービスであるため、後々使えそうです。
...
DC-2はWordPressで動作しており、これはIPアドレスではなくDC-2というドメインでアクセスされることを想定されています。
...
ユーザーネームやパスワードが分からないので、まずは`wig`というWordPressに対する解析ツールを用います。`wig`を用いることでバージョンを絞り込めます。

(今回の攻撃に直接`wig`による解析結果は影響しませんが、WordPressに対する`wig`の解析は非常に有効であることが知られているため、学習目的という意味も含めて実行しました。)
...
先ほどの`Flag 1`ページで`CeWL`を用いるヒントが示されていました。
...
### 5. SSHでのアクセスとユーザー切り替え
ここで、序盤の`nmap`で得たSSHポートが開いていたという情報を思い出し、ダッシュボードと同じパスワードが使い回されている可能性が高いと考え、侵入を試みました。
...
コマンドモードで`:set shell=/bin/bash`と入力し、さらに`:shell`を実行して、設定した通常のbashシェルを直接起動します。

これは、`vi`エディタ自体の実行が許可されていることを逆手に取り、`vi`の内部から直接通常のシェルを呼び出すことで、`rbash`のコマンド入力制限をすり抜けるという手法です。

しかし、シェルが新しくなってもコマンドの探索パス（環境変数 $PATH）は依然としてrbashの制限された状態を引き継いでしまっています。そのため、脱出後以下のコマンドを実行して、本来のパス設定を取り戻します。
```bash
tom@DC-2:~$ export PATH=/bin:/usr/bin:/sbin:/usr/sbin:$PATH
```
これにより、すべての基本コマンドが解放され、`su`コマンドを用いた`jerry`への切り替えが可能になります。
...
今回攻略したDC-2マシンの特に致命的な脆弱性は

- WordPressのログインにWebサイト内に用いられていた単語（CeWLの辞書）から推測できる脆弱なパスワードを用いており、さらにそれをSSHのパスワードに使いまわしていたこと
- `jerry`ユーザーに対して、安易に`git`コマンドの`sudo`、しかもパスワードなしを許可してしまっていたこと

が挙げられます。

### パスワードについて
そもそも`CeWL`とは、ページ内のテキストを収集し、指定の文字数以上の単語を抽出してパスワードクラック用の辞書ファイルを作成するツールです。

近年は汎用的な辞書ファイル(`rockyou.txt`など)を用いた攻撃は対策されていることが多いです。しかし、現実世界の人間は、自身に関係する情報、つまりWebサイトに載っている情報をパスワードとして用いてしまうケースが非常に多いです。`CeWL`はそこを突いたツールです。

今回の実験では`cewl -m 5 -w dict.txt http://dc-2`というコマンドを用いたので、5文字以上の文字列を抽出し、dict.txtにまとめています。

対策として、パスワードをWebサイト上に記載されていない全くランダムな文字列にすることが望ましいです。
また、万が一WordPressのパスワードが突破されたとしてもSSHのパスワードは別にしておくことで、今回のように芋づる式に脆弱性を突かれることを防げます。

### `git`コマンドの権限について
本実験の環境では、設定不備により`jerry`ユーザーに対して`sudo git`コマンドの実行がパスワードなしで許可されていました。
`git`コマンドでヘルプマニュアルなどを表示する際、出力が画面に収まらないと内部で自動的に`less`などのページャーが起動します。このページャーには、`!`記号を用いて実行中のプログラムからOSのシェルを直接呼び出す機能が標準で備わっています。
...
今回の実験のの検証プロセスなどは、以下のGitHubにて公開・更新しています。
